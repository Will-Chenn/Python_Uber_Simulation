...
class DriverRequest(Event):
    ...
    def do(self, dispatcher: Dispatcher, monitor: Monitor) -> List[Event]:
        ...
        if rider is not None:
            ...
            events.append(Pickup(self.timestamp + time,
                                 rider, self.driver))
...
class Cancellation(Event):
    ...
    def __str__(self) -> str:
        ...
        """
        return "{} -- {}: Cancel a rider".format(self.timestamp, self.rider)
...
class Pickup(Event):
    ...
    def do(self, dispatcher: Dispatcher, monitor: Monitor) -> List[Event]:
        ...
        monitor.notify(self.timestamp, DRIVER, PICKUP,
                       self.driver.id, self.driver.location)
        if self.rider.status == WAITING:
            time = Driver.start_ride(self.driver, self.rider)
            events.append(Dropoff(self.timestamp + time, self.driver))
    ...
    def __str__(self) -> str:
        ...
        """
        return "{} -- {}: Pickup a rider".format(self.timestamp, self.rider)
...
class Dropoff(Event):
    ...
    def __str__(self) -> str:
        ...
        """
        return "{} -- {}: Dropoff a rider".format(self.timestamp, self.driver)
    ...
    with open(filename, "r") as file:
        for line in file:
            ...
            if event_type == "DriverRequest":
                locate = deserialize_location(tokens[3])
                driver = Driver(tokens[2], locate, int(tokens[4]))
                event = (DriverRequest(timestamp, driver))
                events.append(event)

            elif event_type == "RiderRequest":
                origin = deserialize_location(tokens[3])
                destination = deserialize_location((tokens[-2]))
                rider = Rider(tokens[2], int(tokens[-1]), origin, destination)
