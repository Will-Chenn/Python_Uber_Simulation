        ...
        if len(self._items) == 0:
            ...
            for index in range(total):
                if item < self._items[index]:
                    ...
                    break
                if item == self._items[index]:
                    num = count_same(item, self._items[index:])
                    self._items.insert(index + num, item)
                    i += len(self._items)
                    break
                i += 1
...
def count_same(num: object, i: list) -> int:
    """
    from list i, counting how many items keep equals to num,
    return the number of how many
    """
    count = 0
    for ch in i:
        if ch == num:
