...
class Monitor:
    ...
    def _average_total_distance(self) -> float:
        ...
        count = []
        num = 0
        for a in self._activities[DRIVER].values():
            for i in range(len(a) - 1):
                dist += abs(a[i].location.row - a[i + 1].location.row) + \
                    abs(a[i].location.column - a[i + 1].location.column)

        for ppl in self._activities[DRIVER]:
            count.append(ppl)
            num = len(count)
        if num == 0:
            return 0
    ...
    def _average_ride_distance(self) -> float:
        ...
        for activities in self._activities[DRIVER].values():
            for index in range(len(activities) - 2):
                if activities[index].description == REQUEST:
                    distance += abs(activities[index + 1].location.row
                                    - activities[index + 2].location.row) + \
                        abs(activities[index + 1].location.column
                            - activities[index + 2].location.column)
